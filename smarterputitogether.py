...
# put angles in range [-180, 180] rather than [0, 360]  (yes, in degrees)
def getinrange(theta):
    if theta < -180:
        theta += 360
    if theta > 180:
        theta -= 360
...
# return an rgb tuple for x in [xmin,xmax]. Do some kind of batshit
# scaling so the map is not stupid.
def colorme(x, xmin, xmax):
    # val0 = 0 and val1 = 1
...
# fromabove determines what kind of picture you make: view of pipes or top
# view of pressure head in m
fromabove = 1  # set to 0 or 1--see below
# figure out number of writes by checking lable on last .txt file in
# directory (this could go wrong if the naming scheme changes)
...
for i in range(Nedges):
    ...
    if(ltype > 1 and rtype == 1):
        o_cyl.append(
            "cylinder{<0.,0,R%d*ro>, <1.,0,R%d*ro>, R%d*ro scale<L%d,1,1> rotate<0,0,ang%d>  translate<x%d,y%d,0> }\n" % (i, i, i, i, i, i, i))
        i_cyl.append(
            "cylinder{<0.0,0,R%d*ri>, <.99,0,R%d*ri>, R%d*ri scale<L%d,1,1> rotate<0,0,ang%d>  translate<x%d,y%d,0> }\n" % (i, i, i, i, i, i, i))
    # open at right end, closed at left
    elif(ltype == 1 and rtype > 1):
        o_cyl.append(
            "cylinder{<0.,0,R%d*ro>, <1.,0,R%d*ro>, R%d*ro scale<L%d,1,1> rotate<0,0,ang%d>  translate<x%d,y%d,0> }\n" % (i, i, i, i, i, i, i))
        i_cyl.append(
            "cylinder{<0.01,0,R%d*ri>, <1.0,0,R%d*ri>, R%d*ri scale<L%d,1,1> rotate<0,0,ang%d>  translate<x%d,y%d,0> }\n" % (i, i, i, i, i, i, i))
    # open at both ends
    else:
        o_cyl.append(
            "cylinder{<0,0,R%d*ro>, <1.,0,R%d*ro>, R%d*ro scale<L%d,1,1> rotate<0,0,ang%d>  translate<x%d,y%d,0> }\n" % (i, i, i, i, i, i, i))
        i_cyl.append(
            "cylinder{<0.0,0,R%d*ri>, <1.0,0,R%d*ri>, R%d*ri scale<L%d,1,1> rotate<0,0,ang%d>  translate<x%d,y%d,0> }\n" % (i, i, i, i, i, i, i))
#######
    ...
    for jj in range(2):
        if jj == 0:
            ...
            if(nodetypes[thisend] == 2):
                try:
                    ...
                    close = rends.index(thisend)
        # find other pipes coming into this node
        relevant = [thing for thing in incoming[thisend]]
        ...
        if(nodetypes[thisend] == 1):
            ...
            theta1 = math.degrees(math.atan2(
                sign * (y[thisend] - y[otherend]), sign * (x[thisend] - x[otherend])))
            theta2 = math.degrees(math.atan2(
                sign2 * (y[thisend] - y[inc]), sign2 * (x[thisend] - x[inc])))
            ...
            print angles
            planes += "plane{<%f,0,0>0 rotate<0,0,%f> translate <%.5f,0,0>}" % (sign, angles[
                                                                                0] / 2., where)
            planes += "plane{<%f,0,0>0 rotate<0,0,%f> translate <%.5f,0,0>}\n" % (
...
for i in range(202, 206):
    ...
    for j in range(Nedges):
        lines.append("#declare fig%d = \"output_data/out%d_" %
                     (j, j) + istring + "\";\n")
    ...
    fout.write("texture{pigment{gradient <0,0,1> color_map {" +
               colormap + cscale + "finish{ambient 1.0 diffuse 0.}}}")

    # write the colorbar
    if fromabove:
        fout.write("#declare cbarmax = %.2f;\n" % cbarsize)
        fout.write("union {box {<0,0,0>,<1,cbarmax,0.001>pigment {gradient y color_map {" +
                   colormap + "}scale cbarmax} finish{ambient 1.0 diffuse 0.}}")

        Nbars = 3  # number of ticks on colorbar
        for j in range(Nbars):
            dy = float(j) * cbarsize / float(Nbars - 1)
            fout.write(
                "box{<-.5,%.2f,0><1.5,%.2f, 0.001> pigment{rgb 1}}\n" % (dy - .1, dy + .1))
