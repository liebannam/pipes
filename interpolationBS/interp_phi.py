...
iphi = lambda x: np.sqrt(9.8/8.)*(1.-np.cos(x))/np.sqrt((x-np.sin(x))*np.sin(x/2.))
...
def main():
    ...
    for k in range(len(alphas)):
        ...
        r[k,0] = abs(fp1[-1])
        r[k,1] = abs(fp2[-1])
        err[k,0] =np.linalg.norm(Phitrue1-cheb.chebval(fX(A1,p,0,a1),fp1))
        err[k,1] =np.linalg.norm(Phitrue2-cheb.chebval(fX(A2,p,a2,a1),fp2))
    ...
    ticks = [1./6., 1./4., 1./3.,5./12., 1./2., 2./3.,3./4., 5./6., 1.];
    plt.xticks(ticks, xlab)
    plt.grid(True)
    plt.title(r'$\phi(x) \approx \tilde{\phi}(x) =\sum_k a_k T_k(cx^{\alpha}-1)$')
