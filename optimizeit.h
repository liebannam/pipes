...
public:
		...
		levmar(M_+1,n), x0(n),Ntwk(Ntwk_i),whichnode(whichnode), Vin(Vin_)
	{
		...
		if (modetype==1){xfake[0] = 2.*Vin/T;}//fourier constant mode = 2Vin/T
		else{xfake[0] = 2*Vin/T-sumQ-1./12.*(x[0]-x[Ndof-1]);}//set Q(0,0) value
...
//	}
	...
	if (modetype==1){xfake[0] = 2.*Vin/T;}//fourier constant mode = 2Vin/T
	else{xfake[0] = 2*Vin/T-sumQ-1./12.*(x[0]-x[Ndof-1]);}//set Q(0,0) value to enforce total volume thing
...
{	 
	...
	for(int i=0; i<m; i++)
	{
		...
		rr[i] = Ntwk.channels[pj]->HofA(a,false)-hdata[i+delay];
